...
- **Double-line cell borders look like a single hairline at normal zoom** on the editor grid. Univer paints the `double` border style (Excel's `s:7`) as two 1px strokes with a 1px gap; below ~110% zoom the pair anti-aliases into what reads as one line. The border data is correct and the **PDF / HTML export renders it as a true CSS `double` line** — this is a live-canvas-only legibility quirk in Univer's renderer, not data loss. Zoom to 110%+ to see the two lines.
- **Conditional-formatting Minimum / Maximum value inputs are disabled while the bound _type_ is "Minimum" or "Maximum".** In Univer's Conditional Formatting panel (e.g. Color Scale), the numeric value box next to each bound is greyed out when the type dropdown is set to "Minimum"/"Maximum" — those derive the value from the data automatically, so there is nothing to type (this matches Excel). To enter a specific value, change the bound's **type dropdown to "Number", "Percent", "Percentile", or "Formula"** and the input enables. Not a bug — Univer's panel behaving as Excel's does.
...
- **Drawing anchor offsets can drift by up to ½ pixel in two edge cases (M18 A3).** A drawing's exact sub-cell anchor offset (stored in EMU in the `.xlsx`) is preserved byte-for-byte on round-trip _as long as the drawing is left untouched in the editor_. Univer works in whole pixels, so Notesheet keeps the original EMU anchor and re-emits it exactly when the drawing hasn't moved. Two cases fall back to a pixel-grid approximation (`pixels × 9525 EMU`) instead, which can differ from the source by at most ~½px (3175 EMU) — visually imperceptible, and never a wrong position or crash: (1) a **sub-pixel nudge** in the editor (a move smaller than one pixel rounds to the same pixel, so it reads as "unmoved" and the _original_ offset is re-emitted); (2) if a future Univer version **re-splits an untouched drawing's cell + offset** (representing the same position with a different cell index and offset), the exact-match check fails and the pixel-grid fallback is used. Whole-cell and pixel-level positioning are always exact; only the sub-pixel remainder is affected.
...
- **iconSet conditional-formatting rules**: cells render with the value but no glyph (arrows, traffic lights, etc.). dataBar, cellIs, top-N, and colorScale all bake into HTML (dataBar as a CSS `linear-gradient` bar).
- **Charts** render as static inline SVG (bar / column / horizontal-bar / line / pie / doughnut) so they survive PDF / HTML export, where the live Chart.js canvas cannot. The SVG uses the shared `CHART_PALETTE`, not per-series Excel colours (tracked separately). A chart legend lays out on a single row with no wrapping, so a chart with many series / very long labels can clip the rightmost legend entries against the SVG edge.
...
| ✅  | Dev guidelines & repo-rules cleanup — consolidate contributor/agent guidelines ([`CONTRIBUTING.md`](./CONTRIBUTING.md), [`AGENTS.md`](./AGENTS.md)) and add enforcement (ESLint + Prettier + typecheck + dependency-downgrade guard, wired to pre-commit hooks and CI) | [#35](https://github.com/kamleshnanda/joplin-notesheet-plugin/pull/35)                                                                                                                                                                                                                      |
| ✅  | M18 — Drawings & charts: charts in HTML / PDF / preview export; image `.xlsx` round-trip; shape (`<xdr:sp>`) preserve-only `.xlsx` round-trip; Rich Text editor fence data-loss fix; chart-importer sheetId-drift fix — backlog groups A–C                             | [#36](https://github.com/kamleshnanda/joplin-notesheet-plugin/pull/36) [#37](https://github.com/kamleshnanda/joplin-notesheet-plugin/pull/37) [#39](https://github.com/kamleshnanda/joplin-notesheet-plugin/pull/39) [#40](https://github.com/kamleshnanda/joplin-notesheet-plugin/pull/40) |
