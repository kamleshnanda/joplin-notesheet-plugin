...
// glob v11+ no longer exports a default `glob.sync` — destructure the
// named export instead. Used once in createPluginArchive() below.
const { globSync } = require('glob');
const execSync = require('child_process').execSync;

// ── Notesheet build stamp ───────────────────────────────────────────────
// Inject a short build identifier (git commit + UTC build time) so the
// running plugin can log exactly which commit is live. The manifest version
// is hardcoded and doesn't change per build, so it can't tell a fresh build
// from a stale one; this stamp does. Exposed as the compile-time global
// `__NOTESHEET_BUILD__` via DefinePlugin (see pluginConfig below) and logged
// once on startup in src/index.ts. Fail-soft: a missing git / detached state
// degrades to 'unknown', never breaks the build.
function notesheetBuildStamp() {
    let commit = 'unknown';
    let dirty = '';
    try {
    ...
    // ISO minute precision is plenty for "which build is this".
    const builtAt = new Date().toISOString().replace(/:\d\d\.\d+Z$/, 'Z');
    return `${commit}${dirty} @ ${builtAt}`;
}
const NOTESHEET_BUILD_STAMP = notesheetBuildStamp();
...
function createPluginArchive(sourceDir, destPath) {
    const distFiles = globSync(`${sourceDir}/**/*`, {
        nodir: true,
...
const pluginConfig = {
    ...
    plugins: [
        // Inject the build stamp as a compile-time constant. `require`d inline
        // (not via the `const webpack` declared further below) so this object
        // literal stays valid regardless of declaration order.
        new (require('webpack').DefinePlugin)({
            __NOTESHEET_BUILD__: JSON.stringify(NOTESHEET_BUILD_STAMP),
        }),
        new CopyPlugin({
            patterns: [
                {
...
const extraScriptConfig = {
    ...baseConfig,
    mode: 'production', // CRITICAL: Must be production to avoid eval() and CSP violations
    resolve: {
        alias: {
            api: path.resolve(__dirname, 'api'),
        },
...
const browserExtraScripts = new Set(['editorView.tsx']);
